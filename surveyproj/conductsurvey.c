...
/* Return 1 (TRUE) iff pcAnswer is a valid concentration. */

static int isValidConcentration(const char *pcAnswer)
{ 
   const char* apcMajors[] = {
      ...
      "LIN", "MAE", "MAT", "MED", "MOD", "MOG", "MOL", "MSE", "MTD",
      "MUS", "NES", "NEU", "ORF", "PAW", "PER", "PHI", "PHY", "PLS",
      "POL", "POP", "POR", "PSY", "QCB", "REL", "RES", "RUS", "SAN",
      "SAS", "SLA", "SML", "SOC", "SPA", "STC", "SWA", "THR", "TPP",
      "TRA", "TUR", "TWI", "URB", "URD", "VIS", "WRI", "WWS",
      "undeclared", "other", "N/A", NULL};
   int i = 0;
 
   assert(pcAnswer != NULL);

   while (apcMajors[i] != NULL)
   {
      if (strcmp(pcAnswer, apcMajors[i]) == 0)
         return 1;
      i++;
   }
   return 0;
...
static int isValidDegree(const char *pcAnswer)
{
   const char *apcDegrees[] =
      {"AB", "BSE", "grad", "undeclared", "N/A", NULL};
   int i = 0;
...
{       
   ...
   handleQuestion(
      "What is your concentration/major "
      "(COS, MAT, ..., undeclared, other, N/A)?",
      isValidConcentration, psSurveyFile);

   printf("\n");

   handleQuestion(
      "What degree are you pursuing (AB, BSE, grad, undeclared, N/A)?",
