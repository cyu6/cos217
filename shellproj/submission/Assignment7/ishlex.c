...
/* The "special" tokens are < and >. All other tokens are "ordinary". 
   Read a line from stdin.  Write the line to stdout. If the line 
   contains non-characters, then write an error message to stderr 
   and reject the line. Otherwise write to stdout each token that 
   the line contains in order of input and their respective type. 
